...
def figtbl(name, params, rows, oasparams):
    ...
    params[2][name+"col2"] = int(params[2][name+"col2"])
    params[3][name+"col2"] = max(1, int(params[3][name+"col2"]))

    vol = float(params[0][name+"col2"])
    prds = int(params[1][name+"col2"])
    total = int(params[2][name+"col2"])
    new_num = int(params[3][name+"col2"])
    ...
    try:
        oas = oas = root_scalar(f, bracket=[0,1], method="brentq")
