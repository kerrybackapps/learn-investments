import dash_bootstrap_components as dbc
from dash import dcc, html, Output, Input, callback
from pages.futures_options.black_scholes_values_figtbl import figtbl
from pages.formatting import Slider, Layout

title = "Black-Scholes plots"
runtitle = None
chapter = "Futures and Options"
chapter_url = "futures-options"
urls = None
text = """
        The Black-Scholes call and put values are presented as functions of the underlying asset price.  Both are 
        convex functions.  As the time to maturity shrinks, the plots will converge to the intrinsic values 
        max(S-K,0) for a call and
        max(K-S,0) for a put, where K denotes the strike price and S denotes the underlying asset price.  The intrinsic
        values are denoted by the red dotted lines.
        """

name = "black-scholes"

inputs = [name + "input" + str(i) for i in range(5)]

slider1 = Slider(
    "Strike price", mn=0, mx=100, step=5, value=50, tick=25, name=inputs[0], kind="dol"
)
slider2 = Slider(
    "Volatility of underlying",
    mn=0,
    mx=80,
    step=1,
    value=40,
    tick=20,
    name=inputs[1],
    kind="pct",
)
slider3 = Slider(
    "Risk-free rate", mn=0, mx=5, step=0.1, value=2, tick=1, name=inputs[2], kind="pct"
)
slider4 = Slider(
    "Dividend yield", mn=0, mx=5, step=0.1, value=4, tick=1, name=inputs[3], kind="pct"
)
slider5 = Slider(
    "Years to maturity", mn=0, mx=5, step=0.1, value=2, tick=1, name=inputs[4]
)

graph1 = dcc.Graph(id=name + "fig1")
graph2 = dcc.Graph(id=name + "fig2")

col1 = dbc.Col(slider1, md=4)
col2 = dbc.Col(slider5, md=4)
row1 = dbc.Row([col1, col2, dbc.Col(md=4)])

col1 = dbc.Col(slider2, md=4)
col2 = dbc.Col(slider4, md=4)
col3 = dbc.Col(slider3, md=4)
row2 = dbc.Row([col1, col2, col3])

col1 = dbc.Col(graph1, md=6)
col2 = dbc.Col(graph2, md=6)
row3 = dbc.Row([col1, col2], align="center")

body = html.Div([row1, row2, html.Br(), row3])

layout = Layout(
    title=title,
    runtitle=runtitle,
    chapter=chapter,
    chapter_url=chapter_url,
    urls=urls,
    text=text,
    body=body,
)
lst = [Output(name + f, "figure") for f in ["fig1", "fig2"]] + [
    Input(i, "value") for i in inputs
]


@callback(*lst)
def call(*args):
    return figtbl(*args)
