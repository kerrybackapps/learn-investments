...
from pages.futures_options.binomial_convergence_figtbl import figtbl
...
name = "binomial-convergence"

inputs = [name + "input" + str(i) for i in range(6)]

slider1 = Slider(
    "Strike price", mn=0, mx=100, step=1, value=50, tick=25, name=inputs[0], kind="dol"
)
slider2 = Slider(
...
slider4 = Slider(
    "Dividend yield", mn=0, mx=5, step=0.1, value=1, tick=1, name=inputs[3], kind="pct"
)
slider5 = Slider(
...
col3 = dbc.Col(slider6, xs=12, sm=6, md=4, lg=4, className="mb-2")
